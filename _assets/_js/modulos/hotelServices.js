...
+'<div class="header"><div class="titles"><div class="nombre"> {name} </div>'
+'</div>'
+'<div class="titles"><div class="fechas" style="width: 100%!important;">'+idioma.comparador.fecha+' {fechas} </div></div></div>'
+'<div><span class="ubica">'+idioma.comparador.destino+'</span> {destino} </div> '
+'<div class="descrip" data-val="{id}" data-url="informacion" style="width: 100%!important;">{HotelDescription}</div></div></div></div>'
+'<div class="ultratabs" id="ultratabs{id}">'
+'<div class="menuHotels">'
+'<ul class="tabs">'
+'<li id="b{id}informacion" class="btabstab info btabs{id}" data-val="{id}" data-url="informacion" data-accion="show" data-tabs="{dataTabs}">'+idioma.comparador.info+'</li>'
	...
	if(location.search!="")
	{
		...
		if(variables.ta=="dt")
		{
			...
			}).done(function(data)
			{
				...
				var last=$("#formHoteles").serializeArray();
				$(".buscadorh [type=text], .buscadorh select").attr("disabled", true);
				$("#pest, .alertBad2, .alertBad, #alertaResultados").hide();
				$("#contenedorHoteles").hide("slow");
				$("#loading,  #frame2").show();
				$("#listado").html("");
				$(document).scrollTop(($("#frame").offset().top - 300));
...
models.CargaHoteles = function(data){
	...
	if(data.hotelesFiltros.length>0){
		...
		$("#frame2").hide();
		$(".alertBad, #alertaResultados").show("slow");
	}
	//$("#loading, #frame2").hide();
}
	...
	let table = "";
		if(totalOpciones.length>0){
			...
			for(let i= 1; i<=totalOpciones.length; i++){
				table = $(`#mostrar${hotelCode} .contenedorHabitaciones .hab_${i} tbody`);
				let selected = table.find(":checked");
				let column = selected.parent().parent();
				let datosHabitacion = {
					"Tipo habitación"	: column.find("td")[1].innerHTML,
					"Plan alimentos" 	: column.find("td")[2].innerHTML,
					"Public price"		: column.find("td")[3].innerHTML,
					"Rewards"			: column.find("td")[4].innerHTML,
					"Precio members"	: column.find("td")[5].innerHTML 
				}
