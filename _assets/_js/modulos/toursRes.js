...
$(document).ready(function(e) {
	$(document).on("click","#botonFiltro",function()
	{
        ...
        $wrapper.find('.hotel').sort(function(a, b)
        {
            
            if($filterType == 1)
            {

            var mayor = $(a).find(".tablaPrecios tr:first-child .special-price").attr("data-price");
            var menor = $(b).find(".tablaPrecios tr:first-child .special-price").attr("data-price");

            }

            if($filterType == 2)
            {
            	var menor = $(a).find(".tablaPrecios tr:first-child .special-price").attr("data-price");
            	var mayor = $(b).find(".tablaPrecios tr:first-child .special-price").attr("data-price");
	   	...
	   	$("#fichasHoteles").find(".hotel").removeClass("animated fadeIn jp-hidden");
	   	paginacion();
	...
	// funcionamiento para las opciones de los destinos de los mys
		...
		$(".smallImg").on("click",function(){
				...
				$( "#startDate" ).prop("value",$.datepicker.formatDate('dd-mm-yy', new Date(calFecha.getFullYear(),calFecha.getMonth(),calFecha.getDate()+2)) );
					
				$( "#endDate" ).prop("value",$.datepicker.formatDate('dd-mm-yy', new Date(calFecha.getFullYear(),calFecha.getMonth(),calFecha.getDate()+3)) );
				...
				if (mys == $(this).data('mys')) {
						$.ajax({
								 url:"_assets/_controllers/tourProcesaBuscadorRes.php",
								 type:"POST",
								 //dataType:"html",
								 data:{valueF :last}
								 }).done(function(data){ 
