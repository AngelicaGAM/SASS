		...
		$("#search").click(
			function(){
				...
				if(tipoViaje==="oneway")
				{
					...
					{ alert("Select your origin from the list"); ban=false; }
					if(($("#flightto_0").val()=="" && ban) || ($("#cTo_0").val()=="" && ban))
					{ alert("Select your destination from the list"); return false; ban=false; }
					if($("#startDate_0").val()=="" && ban)
					{ alert("Select your departure date"); ban=false;  }
					if($("#adult").val()==0 && ban)
				...
				else if(tipoViaje==="roundtrip")
				{
					...
					{ alert("Select your origin from the list"); ban=false; }
					if(($("#flightto_0").val()=="" && ban) || ($("#cTo_0").val()=="" && ban))
					{ alert("Select your destination from the list"); ban=false; }
					if($("#startDate_0").val()=="" && ban)
					{ alert("Select your departure date"); ban=false;  }
					if($("#endDate_0").val()=="" && ban)
					{ alert("Select your arrival date"); ban=false; }
					if($("#adult").val()==0 && ban)
					{ alert("Select adult number"); ban=false; }
				}
				else
				...
				if(ban)
				{
					...
					if(tipoViaje=="oneway" || tipoViaje=="roundtrip")
					  ...
					  else
					  {
					   ...
					   $.ajax({
									 url:"../_assets/_controllers/vuelosProcesaBuscadorTF.php",
									 type:"POST",
									 //dataType:"html",
									 data:{valueF :last}
									 }).done(function(data){ 
