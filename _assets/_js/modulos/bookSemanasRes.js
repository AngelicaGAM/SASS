...
$(document).ready(function (e) {
		...
		$(document).on("click", ".mapadetalle", function () {
			...
			$.ajax({
				...
				/* $(".mapa_" + $id).fancybox({
					afterClose: function () {
						console.log('closed :( ');
						$(".mapa_" + $id).attr('data-fancybox');
					},
					afterShow:function()
					{
						$(".mapa_" + $id).click(function(){return false;});
					}
				}) .click(); */
	...
	$("#buscarPW").click(	function() {
		if($("#SPP").val()!=0) {
			...
			alert("Selecione por lo menos una opcion");
		}
	});
...
function PremiumPicks(idPP) {
	...
	$("#loading").show();
	$.ajax({
		url: "_assets/_controllers/getPremiumPicksRes.php",
		type:"POST",
		dataType:"json",
